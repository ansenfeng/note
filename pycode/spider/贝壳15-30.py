...
def get_html(page):
    """获取网站html代码"""
    url = "https://cd.ke.com/ershoufang/pg{}co41bp15ep30/".format(page)
    headers = {
...
def getdata(html):
    ...
    for r2 in results:
        ...
        for title,address,area,price1,price2,ting,tm,urlq in zip (title,address,area2,price1,price2,ting,tm,urlq):
            sqlc = "INSERT INTO  beike_15_30 (name,address,area,price1,price2,ting,tm,url) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"
...
def creattable():
    sql ="""
    CREATE TABLE if not exists `beike_15_30`(
